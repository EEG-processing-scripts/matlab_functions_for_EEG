 ...
 [ndat nchan]=size(data);
  ...
  if length(epleng)==0
   ...
   if length(freqbins)==0
    ...
    para.segave=1;
    para.subave=0;
    [cs,nave,cs2]=data2cs_event(data,segleng,segshift,epleng,maxfreqbin,para);
   
     
    [nm nf]=size(freqbins);
    psall=zeros(nchan,nchan,nm);
    pssumall=zeros(nchan,nm);
    psall2=zeros(nchan,nchan,nm);
    pssumall2=zeros(nchan,nm);
    for ii=1:nm;
      psall(:,:,ii)=cs2ps(cs(:,:,freqbins(ii,:)));
      pssumall(:,ii)=sum(psall(:,:,ii),2);
      psall2(:,:,ii)=cs2ps(cs2(:,:,freqbins(ii,:)));
      pssumall2(:,ii)=sum(psall2(:,:,ii),2);
    end
    psisum=squeeze(pssumall);
    psisum2=squeeze(pssumall2);
      
    csall=cs;
    csall2=cs2;
    psloc=zeros(nchan,nchan,nepochjack,nm);
    pssumloc=zeros(nchan,nepochjack,nm);
    psloc2=zeros(nchan,nchan,nepochjack,nm);
    pssumloc2=zeros(nchan,nepochjack,nm);
...
%   stdpsisum=stdpssum; 
 ...
 return
    ...
    function [cs,nave,cs2]=data2cs_event(data,segleng,segshift,epleng,maxfreqbin,para);
...
cs=zeros(nchan,nchan,maxfreqbin,nseg);
av=zeros(nchan,maxfreqbin,nseg);
cs2=zeros(nchan,nchan,maxfreqbin,nseg);
av2=zeros(nchan,maxfreqbin,nseg);
mywindow=repmat(hanning(segleng),1,nchan);

dataep2 = reshape(data,epleng,[],size(data,2));
datalocfft2 = fft(dataep2.*permute(repmat(mywindow,[1 1 size(dataep2,2)]), [1 3 2]));
   ...
   dataep=data((j-1)*epleng+1:j*epleng,:);
    for i=1:nseg;
        ...
        for f=1:maxfreqbin % for all frequencies           
            cs(:,:,f)=cs(:,:,f)    +conj(datalocfft(f,:)'*datalocfft(f,:));
            av(:,f)=av(:,f)+conj(datalocfft(f,:)');
            cs2(:,:,f)=cs2(:,:,f)+conj((squeeze(datalocfft2(f,j,:)).')'*squeeze(datalocfft2(f,j,:)).');
            av2(:,f)=av2(:,f)+conj((squeeze(datalocfft2(f,j,:)).')');
            if sum(cs(:) ~= cs2(:)) || sum(av(:) ~=av2(:))
                warning('!')
                pause;
            end
